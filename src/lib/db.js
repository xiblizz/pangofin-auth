...
export function getUserIPs(userId) {
    return db.query('SELECT * FROM user_ips WHERE user_id = ? ORDER BY created_at ASC').all(userId)
}

/** Returns the DB entry for a specific user+ip combo, or null. */
export function getIPEntry(userId, ip) {
    return db.query('SELECT * FROM user_ips WHERE user_id = ? AND ip = ?').get(userId, ip)
}

/** Inserts a new user_ip row and returns its generated id. */
export function addUserIP(userId, ip) {
    const row = db.query('INSERT INTO user_ips (user_id, ip) VALUES (?, ?) RETURNING id').get(userId, ip)
    return row.id
}

/** Records the Pangolin rule id for a tracked ip entry + resource. */
export function addIPRule(userIpId, resourceId, ruleId) {
    db.run('INSERT INTO ip_rules (user_ip_id, resource_id, rule_id) VALUES (?, ?, ?)', [
        userIpId,
        resourceId,
        String(ruleId),
    ])
...
export function getIPRules(userIpId) {
    return db.query('SELECT * FROM ip_rules WHERE user_ip_id = ?').all(userIpId)
}

/** Deletes a user_ip entry (cascades to ip_rules). */
export function removeUserIP(id) {
    db.run('DELETE FROM user_ips WHERE id = ?', [id])
}
